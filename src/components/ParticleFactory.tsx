...
class Particle {
  ...
  update(deltaTime: number, mouseX?: number, mouseY?: number) {
    ...
    if (mouseX !== undefined && mouseY !== undefined) {
      ...
      const dx = scaledMouseX - this.x
      const dy = scaledMouseY - this.y
      ...
      if (distance < LargeRadius) {
        ...
        const largeAngle = Math.atan2(dy, dx)
        repX += Math.cos(largeAngle) * largeDisPercent * -LargeInten * mouseSensitivity
        repY += Math.sin(largeAngle) * largeDisPercent * -LargeInten * mouseSensitivity
      }
...
class ParticleCanvas {
  ...
  private exitAnimationDuration = 1000 // 离场动画持续时间
  private newImageDelay = 100 // 新图片加载延迟
  ...
  drawCanvas() {
    ...
    if (this.ParticleArr.length > 0) {
      ...
      const relativeMouseX = this.mouseX !== undefined ? this.mouseX - particleAreaX : undefined
      const relativeMouseY = this.mouseY !== undefined ? this.mouseY - particleAreaY : undefined
