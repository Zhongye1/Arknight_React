...
export default function PageTracker() {
  ...
  const handleElementAnimation = useCallback(
    (element: RefObject<HTMLDivElement>, timeout: number) => {
      const translateY = $viewIndex > showIndex ? '-translate-y-8' : 'translate-y-8'

      element.current?.classList.add(translateY, 'opacity-0')
      setTimeout(
        () =>
          element.current?.classList.replace(translateY, $viewIndex > showIndex ? 'translate-y-8' : '-translate-y-8'),
        timeout,
      )
  ...
  return (
    <div className="w-[10rem] portrait:w-[4rem] absolute top-[44.4444444444%] portrait:top-[auto] right-[7.375rem] portrait:right-[2.875rem] portrait:bottom-[12.5rem] translate-x-1/2 z-[6] whitespace-nowrap leading-[normal] select-none">
      <div
        ref={nowIndexElement}
        style={{ transitionDuration: nowIndexDuration + 'ms' }}
      ...
      <div
        ref={tinyIndexElement}
        style={{ transitionDuration: tinyIndexDuration + 'ms' }}
      ...
      <div
        ref={microInfoElement}
        style={{ transitionDuration: microInfoDuration + 'ms' }}
      ...
      <div
        ref={labelElement}
        style={{ transitionDuration: labelElementDuration + 'ms' }}
