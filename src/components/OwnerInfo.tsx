...
function Divider({ children, portraitHidden }: { children: React.ReactNode; portraitHidden?: boolean }) {
  return (
    <div
      className={
        'mr-[1.25rem] portrait:mr-[6.75rem] ml-[2.25rem] portrait:ml-[8rem] whitespace-nowrap' +
...
function Welcome() {
  const name = arknightsConfig?.navbar?.ownerInfo?.name ? (
    <div className="pb-5">
      <img
        ...
        alt="通行证"
      />
      <div className="text-[1rem] portrait:text-[1.75rem] mt-[-3.5rem] portrait:mt-[-5.75rem] ml-8 portrait:ml-[6.5rem]">
        <div
          className="h-6 portrait:h-8 bg-[#232323] px-3 inline-flex items-center"
  ...
  const slogan = arknightsConfig?.navbar?.ownerInfo?.slogan ? (
    <div className="w-[10.75rem] portrait:w-[17.75rem] text-[1rem] portrait:text-[1.625rem] font-bold mt-8 ml-11 portrait:ml-[8.5rem] break-words hyphens-auto text-ellipsis">
      {/* TODO: 换个字体 */}
      {arknightsConfig.navbar.ownerInfo.slogan}
    </div>
  ) : (
    <img
      className="w-[10.75rem] portrait:w-[17.75rem] h-auto ml-[2.875rem] portrait:ml-[8rem] block"
...
function ToolBox() {
  ...
  const aClassName =
    'text-inherit no-underline decoration-0 cursor-pointer flex flex-1 opacity-70 transition-opacity duration-300'
  const iconClassName = 'h-auto m-auto pointer-events-none'
  ...
  return (
    <>
      ...
      <div className={'mt-4 pl-4 grid grid-cols-3 gap-y-2 portrait:hidden'}>
        {GitHub && (
          <a target="_blank" href={GitHub} className={aClassName} aria-label="GitHub" rel="noreferrer">
            <IconGitHub className={'w-[2rem] ' + iconClassName} />
          </a>
        )}
        {Skland && (
          <a target="_blank" href={Skland} className={aClassName} aria-label="Skland - 森空岛" rel="noreferrer">
            <IconSkland className={'w-[2rem] ' + iconClassName} />
          </a>
        )}
        {Bilibili && (
          <a target="_blank" href={Bilibili} className={aClassName} aria-label="Bilibili - 哔哩哔哩" rel="noreferrer">
            <IconBiliBili className={'w-[3rem] ' + iconClassName} />
          </a>
        )}
        {WeChat && (
          <a target="_blank" href={WeChat} className={aClassName} aria-label="WeChat - 微信" rel="noreferrer">
            <IconWechat className={'w-[2rem] ' + iconClassName} />
          </a>
        )}
        {Weibo && (
          <a target="_blank" href={Weibo} className={aClassName} aria-label="Weibo - 微博" rel="noreferrer">
            <IconWeibo className={'w-[2rem] ' + iconClassName} />
          </a>
        )}
        {TapTap && (
          <a target="_blank" href={TapTap} className={aClassName} aria-label="TapTap" rel="noreferrer">
...
export default function OwnerInfo(): React.JSX.Element {
  ...
  function CloseButton(): React.ReactElement {
    const xLine: string =
      'w-[.375rem] portrait:w-[.25rem] h-4/5 bg-current' +
      ' absolute top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2'
    return (
      <div
        className={
          'portrait:w-[4.5rem] h-[11.25rem] portrait:h-[4.5rem] mt-[3.75rem] portrait:mt-[.5rem]' +
  ...
  return (
    <aside
      className={'w-full h-full absolute top-0 left-0 z-[24] overflow-hidden transition-opacity duration-300'}
      style={{ opacity: $isOwnerInfoOpen ? 1 : 0, visibility: $isOwnerInfoOpen ? 'visible' : 'hidden' }}
    >
      <div
        className={
          'w-full h-full absolute top-0 left-0 portrait:bg-black portrait:opacity-70' +
      ...
      <div
        className={
          'w-[14.75rem] portrait:w-[32.5rem] h-full portrait:h-auto bg-ark-blue overflow-hidden' +
      ...
      >
        <div className="w-full h-full absolute top-0 left-0">
          <div
            className={
              'h-[59.375rem] portrait:h-[55rem] text-[#06bbff]' +
        ...
        <div className={'w-full absolute left-0 bottom-0 portrait:static portrait:mb-[3.75rem]'}>
          <div
            className={
              'relative portrait:static pl-[2.5rem] pr-[1.25rem]' + ' portrait:pl-[8rem] portrait:pr-[6.75rem]'
            }
          >
            {
