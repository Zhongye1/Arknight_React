...
import { viewIndex, readyToTouch, isScrollLocked } from '../../components/store/rootLayoutStore'
      ...
      >
        ...
        >
          ...
          <div className="absolute top-1/4 left-16 z-20">
            <h2 className="text-white text-xl font-bold mb-6 tracking-tighter">ABOUT TERRA</h2>
            <div className="flex flex-col gap-4">
              {CATEGORIES.map((cat) => (
                ...
                >
                  <div
                    className={`w-3 h-3 border border-white/50 mr-3 flex items-center justify-center transition-all ${
                      currentCat === cat.id ? 'bg-white' : ''
                    }`}
                  >
                  ...
                  <span
                    className={`text-xs tracking-widest transition-colors ${
                      currentCat === cat.id ? 'text-white' : 'text-gray-500 group-hover:text-gray-300'
                    }`}
                  >
                    {cat.en}
                  </span>
          ...
          <div className="absolute inset-0 flex items-center justify-center pointer-events-none">
            <div className="relative min-w-[1000px] h-[600px]">
              ...
              <div className="absolute inset-0 transition-all duration-700">
                {CATEGORIES.map((cat) => (
                  <img
                    key={cat.id}
                    src={cat.activeImg}
                    className={`absolute z-30 inset-0 w-full h-full object-contain transition-opacity duration-500 ${
                      currentCat === cat.id ? 'opacity-100' : 'opacity-0'
                    }`}
                    alt={cat.id}
                  />
                ))}
              ...
              <div className="absolute inset-0 transition-all duration-700">
                {CATEGORIES.map((cat) => (
                  <img
                    key={cat.id}
                    src={cat.logo}
                    className={`absolute z-40 inset-0 w-full h-full object-contain transition-opacity duration-500 ${
                      currentCat === cat.id ? 'opacity-100' : 'opacity-0'
                    }`}
                    alt={cat.id}
                  />
              ...
              <div
                className={`${
                  currentCat === 'monster_siren' ? 'opacity-100 scale-100' : 'opacity-0 scale-110'
                } transition-all duration-500 ease-in-out origin-center absolute inset-0 `}
              >
                <div>
                  <div
                  ...
                  <div
                    ...
                    style={{
                      ...
                      transition: 'all 700ms ease-in-out, box-shadow 800ms ease-out',
                    }}
                  />
                </div>
              </div>
              ...
              <div
                className={`${
                  currentCat === 'gallery' ? 'opacity-100 scale-100' : 'opacity-0 scale-110'
                } transition-all duration-500 ease-in-out origin-center absolute inset-0 `}
              >
                <div>
                  <div
              ...
              <div
                className={`${
                  currentCat === 'operator' ? 'opacity-100 scale-100' : 'opacity-0 scale-110'
                } transition-all duration-500 ease-in-out origin-center absolute inset-0 `}
              >
                <div>
                  <div
              ...
              <div
                className={`${
                  currentCat === 'video' ? 'opacity-100 scale-100' : 'opacity-0 scale-110'
                } transition-all duration-500 ease-in-out origin-center absolute inset-0 `}
              >
                <div>
                  <div
          ...
          <div className="absolute bottom-24 left-16 z-40 max-w-md pointer-events-auto">
            ...
            <div key={activeData.id} className="flex flex-col items-start">
              ...
              {/* 2. 中文标题 + 装饰条 - 延迟 100ms */}
              <div className="flex items-end gap-4 mt-2 mb-8 animate-enter" style={{ animationDelay: '100ms' }}>
                <h2 className="text-white min-w-[200px] text-7xl font-bold">{activeData.cn}</h2>
              ...
              {/* 4. 按钮 - 延迟 300ms */}
              <div className="relative mt-10 animate-enter" style={{ animationDelay: '300ms' }}>
                <button
                ...
                >
                  <div className="flex flex-col justify-center">
                    <div className="font-bold text-[1.25rem] leading-tight">查看详情</div>
                    <div className="font-semibold text-[0.875rem] leading-tight">READ MORE</div>
                  </div>
                  <div className="ml-auto flex items-center justify-center w-4 h-4 group-hover:translate-x-1 transition-transform">
                    <svg width="7" height="15" viewBox="0 0 7 15" className="block w-full h-auto pointer-events-none">
                      <path d="M1 1 L6 7.5 L1 14" stroke="currentColor" strokeWidth="1.5" />
                    </svg>
                  </div>
                </button>
              </div>
