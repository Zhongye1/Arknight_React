...
import { viewIndex, readyToTouch, isFooterVisible } from '../../components/store/rootLayoutStore'
            ...
            {AK_CARDS.map((card, index) => (
              ...
              >
                ...
                <div className="absolute inset-0 z-10 flex flex-col justify-end p-10 pb-24 md:p-12 md:pb-32">
                  {/* 简化示例，保持你的原始内容 */}
                  <h3 className="text-white text-4xl font-black">{card.title}</h3>
                  <div className="text-white/60 text-sm font-bold mb-8">{card.subtitle}</div>
                ...
                <div className="absolute top-10 left-10 z-20 opacity-0 group-hover:opacity-100 transition-opacity duration-500 delay-200">
                  <span className="text-6xl font-black text-white/5 select-none">{card.id}</span>
                </div>
              </div>
            ))}
          </div>
