...
export default function Operator() {
  ...
  const currentOp = useMemo(() => arknightsConfig.rootPage.OPERATOR.data[currentIndex], [currentIndex])
    ...
    >
      ...
      <div className="OPS_info_block mb-0 h-full flex items-center">
        ...
        <div className="absolute bottom-[-7rem] z-20 h-full flex flex-col justify-center pl-[6rem] portrait:pl-[2rem] w-[45rem] portrait:w-full">
          <AnimatePresence mode="wait">
            ...
            >
              ...
              <div className="flex items-center gap-3 mb-6">
                <span className="bg-white text-black font-black px-2 py-0.5 text-sm tracking-tighter">PROFILE</span>
                <span className="text-white/40 font-benderRegular text-xs tracking-[0.2em]">RHODES ISLAND ://</span>
              </div>
              ...
              <div className="relative mb-8 flex items-end">
                ...
                <div className="ml-6 h-20">
                  <img src={currentOp.logo} className="block w-auto h-full" alt="faction" />
                </div>
              </div>

              {/* CV 语音展示 */}
              <div className="voice-section group" onClick={handlePlayVoice} data-playing={isPlaying}>
          ...
          {/* 4. 底部干员选择列表 (Swiper) */}
          <div className="mt-7">
            <div className=" relative overflow-hidden [list-style:none] p-0 block box-border mx-2 pl-4 my-0 w-[33.625rem] [mask:linear-gradient(-90deg,_rgba(255,_255,_255,_0),_rgb(255,_255,_255)_2rem)] operator-list-container">
                  ...
                  {arknightsConfig.rootPage.OPERATOR.data.map((op, index) => (
                    ...
                    >
                      <div
                        ...
                        className={`thumbnail ${currentIndex === index ? 'active' : 'inactive'}`}
                      >
                        ...
                        <div className="[filter:drop-shadow(rgb(0,_0,_0)_-0.5rem_0.5rem_1rem)] overflow-hidden">
                          <img src={op.portrait} className="block w-full h-auto mx-[auto] my-0" alt={op.cnName} />
                        </div>
                        <div className="name-label [filter:drop-shadow(rgb(0,_0,_0)_0px_0px_0.5rem)_drop-shadow(rgb(0,_0,_0)_0px_0px_0.5rem)] [transition:color_0.3s] text-[rgb(255,_255,_255)]">
                          <div className="relative block font-[SourceHanSans-Medium]">{op.cnName}</div>
                        </div>
                        {/* {currentIndex === index && (
