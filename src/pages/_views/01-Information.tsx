          ...
          {current.href && (
            ...
            >
              <div className="transition-transform duration-300 group-hover:translate-x-1">
                <div className="text-[1.25rem] font-black leading-none">更多情报</div>
    ...
    <div className="w-full h-full relative">
      ...
      <div className="absolute top-0 left-0 w-full h-full flex items-center justify-center">
        <div className="text-[1.25rem] font-bold text-white">
          {title}
          <div className="mt-2 text-[1rem] font-benderRegular text-white">{subtitle}</div>
        </div>
      </div>
    </div>
  )
      ...
      >
        {data.map(({ title, subtitle, href, image }, index) => (
          <SwiperSlide key={index}>
            <a target="_blank" {...{ href }} className="block w-full h-full overflow-hidden">
              {image ? <ImageSlide {...{ title, image }} /> : <TextSlide {...{ title, subtitle: subtitle ?? '' }} />}
            </a>
          </SwiperSlide>
        ))}
      </Swiper>
